...
import ContextMetaShadowPanel from "@/components/ContextMetaShadowPanel";
import EntryShieldX from "@/components/EntryShieldX";
import EntryZoneProgressionPanel from "@/components/EntryZoneProgressionPanel";
    ...
    <div className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/70 shadow-2xl shadow-black/20">
      ...
      {open && <div className="border-t border-slate-800">
        <div className="grid gap-2 p-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Tab active={group === "LEARNING"} icon={<Brain size={15}/>} title="4. Aprendizaje" text="Memoria, edge, continuación post-TP1, walk-forward, rolling y veto shadow" onClick={() => setGroup("LEARNING")} />
        </div>
        ...
        <div className="border-t border-slate-800 pb-4">
          ...
          {group === "LEARNING" && <><VerdictLearningLab symbol={symbol} /><FusionEdgeResearchPanel /><Tp1ContinuationResearchPanel /><ShadowOutcomeModelPanel /><WalkForwardPanel /><RollingContextValidationPanel /><ContextMetaShadowPanel /><GraduatedVetoShadowPanel /></>}
        </div>
      </div>}
    </div>
  </section>;
