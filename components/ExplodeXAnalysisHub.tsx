...
function Tab({ active, icon, title, text, onClick }: { active: boolean; icon: React.ReactNode; title: string; text: string; onClick: () => void }) {
  return <button
    type="button"
    onClick={onClick}
    className={`rounded-2xl border p-4 text-left transition ${active ? "border-cyan-500/30 bg-cyan-500/[.06]" : "border-slate-800 bg-slate-950/50 hover:border-slate-700"}`}
  >
    <div className={`flex items-center gap-2 text-xs font-black ${active ? "text-cyan-200" : "text-slate-300"}`}>{icon}{title}</div>
