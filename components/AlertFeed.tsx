...
export default function AlertFeed() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      } catch { if (mounted) { setConnected(false); setLoading(false); } }
      finally { if (mounted) timer = setTimeout(load, 5000); }
    };
    load();
  ...
  return <>
    ...
    <div className="mt-5 grid gap-3 xl:grid-cols-2">
      {alerts.map((alert) => { const colors = tone(alert.severity); return <article key={alert.id} className={`rounded-2xl border p-4 ${colors.card}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><CircleAlert size={17} className={colors.icon}/><h2 className="font-black text-white">{alert.title}</h2></div><p className="mt-2 text-xs leading-5 text-slate-300">{alert.message}</p></div><div className={`rounded-full border px-3 py-1 text-[10px] font-black ${colors.badge}`}>{severityEs(alert.severity)}</div></div><div className="mt-3 text-[10px] text-slate-600">{new Date(alert.created_at).toLocaleString("es-GT")}</div></article>; })}
