...
type HistoryRow = StrategyMeta & { id:number; symbol:string; side:string; leverage:number; entry_price:number; exit_price:number|null; margin_used?:number|null; opened_at:string; closed_at:string|null; exit_reason:string|null; gross_pnl:number|null; net_pnl:number|null; fees:number|null; slippage:number|null; funding_estimate:number|null };
...
export default function PaperPortfolioLab(){
 ...
 const accountRoi=summary.starting_balance>0?delta/summary.starting_balance*100:null;
 ...
 return <article className="terminal-panel overflow-hidden">
  <div className="border-b border-slate-800/80 p-4">
   <div className="flex flex-wrap items-start justify-between gap-3">
    ...
    <div className="flex items-center gap-2"><span className={`rounded-xl border px-3 py-1.5 text-sm font-black ${Number(roiMargin??0)>=0?"border-emerald-500/30 bg-emerald-500/[.08] text-emerald-300":"border-rose-500/30 bg-rose-500/[.08] text-rose-300"}`}>ROI {roiMargin==null?"—":pct(roiMargin)}</span><span className="rounded-xl border border-cyan-500/25 bg-cyan-500/[.05] px-3 py-1.5 text-sm font-black text-cyan-200">x{p.leverage}</span><Link href={`/coin/${p.symbol}?position=${p.id}`} className="inline-flex items-center gap-1 rounded-xl border border-slate-700 px-3 py-2 text-[10px] font-black text-slate-300 hover:border-cyan-500/30 hover:text-cyan-200">VER EN VIVO <ExternalLink size={11}/></Link></div>
   </div>
  </div>
  <div className="p-4">
   ...
   <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
    ...
    <Cell label="ROI sobre margen" value={roiMargin==null?"—":pct(roiMargin)} good={Number(roiMargin)>=0} bad={Number(roiMargin)<0}/>
