...
type Position = StrategyMeta & { id:number; symbol:string; side:"LONG"|"SHORT"; leverage:number; entry_price:number; mark_price:number; stop_loss:number; take_profit:number; margin_used:number; unrealized_pnl:number; opened_at:string };
...
export default function PaperPortfolioLab(){
 ...
 return <div className="space-y-5">
  ...
  <section className="grid gap-3 lg:grid-cols-[1fr_.42fr]"><div><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black text-white">Posiciones que ExplodeX decidió abrir</h2><span className="text-[10px] text-slate-600">actualiza cada 10 s</span></div><div className="grid gap-3 xl:grid-cols-2">{summary.open_positions.map(p=><article key={p.id} className="terminal-panel p-4"><div className="flex items-start justify-between gap-3"><div><Link href={`/coin/${p.symbol}`} className="text-xl font-black text-white hover:text-cyan-300">{p.symbol}</Link><div className="mt-1 flex flex-wrap items-center gap-2"><span className={`inline-flex items-center gap-1 text-xs font-black ${p.side==="LONG"?"text-emerald-400":"text-rose-400"}`}>{p.side==="LONG"?<ArrowUpRight size={14}/>:<ArrowDownRight size={14}/>} {p.side}</span><span className="rounded-full border border-cyan-500/20 bg-cyan-500/[.05] px-2 py-0.5 text-[9px] font-black text-cyan-200">{strategyLabel(p.strategy_mode)}</span></div></div><div className="rounded-xl border border-slate-700 px-3 py-1.5 text-sm font-black text-cyan-200">{p.leverage}x</div></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Cell label="Entrada" value={px(p.entry_price)}/><Cell label="Mark" value={px(p.mark_price)}/><Cell label={p.stop_survival_enabled?"Hard stop":"STOP"} value={px(p.hard_stop??p.stop_loss)} bad/><Cell label="TP" value={px(p.take_profit)} good/></div>{p.strategy_mode==="STRUCTURE_RETEST_PAPER"&&<div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4"><Cell label="Stop suave" value={px(p.soft_invalidation_stop)}/><Cell label="Score patrón" value={p.pattern_score==null?"—":Number(p.pattern_score).toFixed(1)}/><Cell label="R/R neto" value={p.net_rr==null?"—":Number(p.net_rr).toFixed(2)}/><Cell label="Riesgo stop" value={money(p.actual_stop_risk_usdt)}/></div>}<div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs"><span className="text-slate-500">Margen {money(p.margin_used)}</span><span className={`font-black ${p.unrealized_pnl>=0?"text-emerald-300":"text-rose-300"}`}>{p.unrealized_pnl>=0?"+":""}{money(p.unrealized_pnl)}</span></div><div className="mt-2 flex flex-wrap gap-3 text-[10px] text-slate-600"><span><Clock3 className="mr-1 inline" size={11}/>Abierta {dt(p.opened_at)}</span><span className="text-cyan-300/80">Horizonte {horizonLabel(p)}</span><span>Máx. plan {holdLabel(p.max_hold_minutes)}</span>{p.planned_max_leverage!=null&&<span>Tope lev. {p.planned_max_leverage}x</span>}{p.phase&&<span>Fase {String(p.phase).replaceAll("_"," ")}</span>}{p.stop_survival_enabled&&<span className="text-amber-300/80">mecha en stop suave tolerada; hard stop fijo</span>}</div></article>)}{!summary.open_positions.length&&<div className="terminal-panel col-span-full p-8 text-center text-sm text-slate-600">Sin posiciones abiertas. PAPER espera una autorización ejecutable del Heart.</div>}</div></div>
