...
import { Activity, ArrowRight, BrainCircuit, ShieldAlert } from "lucide-react";
...
function layerTone(state?: string) {
  ...
  if (["YES", "SUPPORT", "SAFE", "CLEAR", "HIT", "STRONG", "FAVORED", "ENTER_NOW"].includes(s)) return "border-emerald-400/20 bg-emerald-400/[.04] text-emerald-100";
  if (["WAIT", "MIXED", "COMPRESSION", "TREND", "WATCH", "WAIT_NEAR"].includes(s)) return "border-cyan-400/20 bg-cyan-400/[.04] text-cyan-100";
  if (["CONFLICT", "RISK", "BLOCKED", "NO", "HIGH_VOLATILITY", "AVOID_OR_COLD"].includes(s)) return "border-rose-400/20 bg-rose-400/[.04] text-rose-100";
...
export default function PredictionStackV5Panel({ symbol }: { symbol: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const analysis = await getLiveAnalysis(safe);
        const value = (analysis.prediction as any)?.prediction_stack_v5 as Stack | undefined;
        if (!dead) {
          setStack(value ?? null);
          setError(null);
  ...
  if (!stack) {
    return <section className="mx-auto mb-4 max-w-[1680px] px-3 sm:px-5 lg:px-6"><div className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4 text-xs text-slate-500"><BrainCircuit size={14} className="mr-2 inline animate-pulse"/>Organizando señales del mercado…</div></section>;
  }
    ...
    <section className="mx-auto mb-4 max-w-[1680px] px-3 sm:px-5 lg:px-6">
      <div className="overflow-hidden rounded-3xl border border-violet-400/15 bg-[radial-gradient(circle_at_top_left,rgba(139,92,246,.10),transparent_34%),linear-gradient(135deg,rgba(5,10,22,.99),rgba(2,7,15,.99))] shadow-2xl shadow-black/20">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 px-5 py-4">
          <div>
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] text-violet-300"><BrainCircuit size={15}/> Prediction Stack v5</div>
        ...
        <div className="grid gap-4 p-5 xl:grid-cols-[.82fr_1.18fr]">
          <div className={`rounded-3xl border p-5 ${decisionTone(decision.state)}`}>
            <div className="text-[10px] font-black uppercase tracking-[.18em] opacity-60">Decisión maestra</div>
          ...
          <div>
            <div className="mb-3 text-[10px] font-black uppercase tracking-[.14em] text-slate-500">Cadena de decisión</div>
            ...
            <div className="mt-3 flex items-center gap-2 rounded-2xl border border-slate-800 bg-black/15 p-3 text-[10px] leading-5 text-slate-500"><ArrowRight size={13}/>El stack organiza evidencia existente. No convierte un score en probabilidad ni fuerza operaciones para producir más señales.</div>
          </div>
        </div>
      </div>
    </section>
