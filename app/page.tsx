...
export default async function HomePage() {
  ...
  const directMarket = marketResult.status === "fulfilled" ? marketResult.value : null;
  const fallbackMarket = opportunities?.market_context as Record<string, any> | undefined;
  const market = directMarket ?? fallbackMarket ?? null;
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-5 border-b border-slate-800 pb-6 md:flex-row md:items-end md:justify-between">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-slate-300"><div className="text-xs uppercase tracking-[0.16em] text-slate-500">Modo</div><div className="mt-1 font-bold text-amber-300">PAPER TRADING</div></div>
      </header>
      ...
      {!opportunities && (
        <section className="mt-6 rounded-3xl border border-amber-500/30 bg-amber-500/5 p-5 text-amber-100">
          <div className="flex items-center gap-2 font-bold"><AlertTriangle size={19} /> No se pudo leer el ranking del backend</div>
          <p className="mt-2 text-sm text-amber-200/70">Verifica en Vercel la variable <code className="rounded bg-black/30 px-1.5 py-0.5">NEXT_PUBLIC_API_BASE_URL</code> y que Railway esté activo.</p>
        </section>
      )}
...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="rounded-3xl border border-slate-800 bg-slate-950/65 p-4"><div className="flex items-center gap-2 text-slate-500">{icon}<span className="text-xs uppercase tracking-[0.12em]">{label}</span></div><div className="mt-3 text-xl font-black text-white">{value}</div></div>; }
function TierSection({ title, subtitle, items }: { title: string; subtitle: string; items: any[] }) { return <section className="mt-10"><div className="mb-4"><h2 className="text-xl font-black text-white">{title}</h2><p className="mt-1 text-sm text-slate-500">{subtitle}</p></div>{items.length ? <div className="grid gap-4 xl:grid-cols-2">{items.map((item) => <OpportunityCard key={item.id} item={item} />)}</div> : <div className="rounded-2xl border border-dashed border-slate-800 px-5 py-6 text-sm text-slate-600">Sin oportunidades en este nivel.</div>}</section>; }
